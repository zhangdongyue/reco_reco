$('document').ready(function(){
  $.fn.extend({
    "sqiRobot": function (option) {
      ...
      if(typeof(option.userNick)=='undefined' || typeof(option.userCode)=='undefined' || typeof(option.chatBu)=='undefined' || typeof(option.productType)=='undefined')
      {
        ...
        console.log("buc");
        $.ajax({
          url:"//"+hostName+"/robot/api/findByCode.json",
          data:{
  ...
  var ASRconfig = {
    config : {
      "url" : "wss://sqi.alibaba-inc.com/asr?app=et&token=xxx&streaming=true&streaming=true" ,
      "constraints" : {
        "audio": true,
        "video": false
      }
  ...
  if(document.domain == 'localhost'){
    ASRconfig.config.url = "ws://localhost:8082/asr?app=et&token=xxx&streaming=true"
  }
  ...
  var RobotObj = {
    ...
    setRobotHtml: function(target,option){
      ...
      var initHtml =
        ...
        "<div class='chat-bubble'>"+
        "<div class='chat-text'>" +
        "<div class='title'><i class='fa fa-question-circle-o'></i>热门问题</div>"+
        "<ul class='hot-ul'></ul>" +
        "</div>"+
        "</div>"+
        "</li>"+
        ...
        "<a class='over-link'>结束人工服务</a>"+
        "</div>"+
        "</div>"+
        "<a class='sendDingding'>催他一下</a>" +
        "</div>"+
  ...
  function checkSyetem(){
    ...
    if(isMac)
    {
      $('#chatInput').attr('placeholder','请描述遇到的问题，command+v可粘贴图片');
    }
    else
    {
      $('#chatInput').attr('placeholder','请描述遇到的问题，ctrl+v可粘贴图片');
    }
  }
  ...
  function bindEvent(){
    ...
    $('.sendDingding').click(function(e){
      $.ajax({
        ...
        jsonp:"callback",
        success:function(d){
          var newHtml ="<li class='center-info'><div class='li-message'>钉钉催促成功！</div></li>";
    ...
    $('.over-link').click(function(e){
      ...
      $.ajax({
        url:"//"+hostName+"/robot/api/EndManualChatSession.json",
        data:{
          product_type:robotProductType,
          chatBu:chatBu,
      ...
      if(!is_recording){
        asrClient = new ASRClient(ASRconfig  , function(msg){
          ...
          if(d != '' && d != null){
            ...
            setTimeout("$('.sendDingding').css('display','none');$('.sendDingding').html('催他一下');",
              5000);
            $('#chatInput').attr('placeholder','请描述遇到的问题，command+v可粘贴图片');
          }else{
            $('#chatInput').val('');
  ...
  function dragRobot(){
    ...
    if(typeof($(element).draggable)=='function'){
      $(".robot-parent").draggable({ cursor: "crosshair",handle: ".chat-title" });
    }
    else{
                ...
                if(typeof(res.arg)!='undefined' && res.msgType=="ManualStatus"){
                  ...
                  if(content!=""){
                    ...
                    if(systemFlag==1){
                      ...
                      var answer=$(answerTemplate).html().replace("<img","<span class='triangle'></span><img").replace("$userNick", currentTime).replace('data="$answerImg"',"src='"+userImg+"'").replace("$currentTime",nick);
  ...
  function setChatResponse(query,flag){
    if(query!=""){
      ...
      if(query.indexOf("href")==-1 && query.split("http").length<=2){
        //转换里面的链接
      ...
      //机器人回复，展现正在输入的提醒
      if(is_manual==false){
        var defaultAnswer="<span id='robotInputTips'>正在输入<span class='robot-dotting'></span></span>";
        var currentTime = "";
        var answer = $(answerTemplate).html().replace("<img","<span class='triangle'></span><img").replace('$chatText',defaultAnswer).replace("$userNick",currentTime).replace('data="$answerImg"',"src='//sqi.alicdn.com/sqi-pub/pro/assets/si/robot/css/img/robot.png'").replace('$currentTime',robotNick);
      ...
      $.ajax({
        url:"//"+hostName+"/robot/api/chatBoxGetResponse.htm",
        data:{
          is_manual:is_manual,
          product_type:robotProductType,
          chatBu:chatBu,
        ...
        success:function(d){
          if(is_manual==true && manual_info==false && customer){//人工答疑
            ...
            for(var i=0;i<userList.length;i++){
              if(d.productInfo.hideNick){
                userContent = userContent + "<span data-widget='popuser' class='popuser'  data-id='" + userList[i].id + "' data-name='"+userList[i].nickOrRealName+"'>"+"客服"+(i+1)+"</span> ";
              }else {
                userContent = userContent + "<span data-widget='popuser' class='popuser'  data-id='" + userList[i].id + "'>" + userList[i].nickOrRealName + "</span> ";
              }
            }
            manual_info =true;
            var newHtml ="<li class='center-info'><div class='li-message'>客服："+userContent+"即将为您服务</div></li>";
            $('.chat-ul').append(newHtml);
            setHeight(500);
            showImg();
          }
            ...
            if(typeof(d.value)!='undefined' && typeof(d.value.result)!='undefined'){
              ...
              if(converter != "" && typeof (converter) != "undefiend"){
                defaultAnswer = converter.makeHtml(defaultAnswer);
              }
              if(!checkHtml(defaultAnswer)){
                //转换自动回复中的链接为可点击的a标签
              ...
              if(d.need_manual!=true && customer){
                var newHtml ="<li class='center-info'><div class='li-message '>没有解决您的问题？您可以<a class='J_change_manual' style='cursor:pointer;'>转人工客服</a></div></li>";
                $('.chat-ul').append(newHtml);
              }
              setReadStatus(); //全部已读
